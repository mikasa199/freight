...
CREATE TABLE `cargo`(
    ...
    `cargo_weight`      DECIMAL NOT NULL COMMENT '货物重量',
    ...
    `distance`		DECIMAL NOT NULL COMMENT '距离（km）',
    `value`             DECIMAL NOT NULL COMMENT '运费价格，元/吨（1000kg）',
    `begin_time`        DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '开始时间',
    `end_time`          DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '结束时间',
    `stock`             DECIMAL NOT NULL COMMENT '库存（kg）',
...
CREATE TABLE `order`(
    ...
    `boss_id`           BIGINT(20) NOT NULL COMMENT '老板id',
    `driver_id`         BIGINT(20) NOT NULL COMMENT '司机id',
    `cargo_weight`      DECIMAL NOT NULL COMMENT '单次运输货物重量',
    `value`             DECIMAL NOT NULL COMMENT '运费',
    `state`             INT NOT NULL DEFAULT 0 COMMENT '订单状态 -1:库存未扣减 0:订单已接收 1:雇主已付款 2:司机正在运货 3:货物已送达目的地 4:司机获取订单的佣金 5:订单完成 6:订单取消',
    `info`              VARCHAR(64) CHARACTER SET utf8mb4 DEFAULT NULL COMMENT '备注信息',
    `created_time`        TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '订单创建时间',
    `updated_time`        TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '订单更新时间',
    PRIMARY KEY (`id`),
...
CREATE TABLE `authentication`(
    `id`                         BIGINT(20) NOT NULL AUTO_INCREMENT COMMENT '自增id',
    `authentication_id`		 BIGINT(20) NOT NULL COMMENT '认证信息id',
    `driver_id`                  BIGINT(20) NOT NULL COMMENT '司机id',
